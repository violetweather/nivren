...
Use Rust 1.85.0, the minimum supported toolchain. Changes to `Cargo.toml`, the VS Code package, or fuzz dependencies must commit the corresponding lockfile. Before a release, also run the release benchmark gate and `niv release check .`; the latter is expected to remain blocked until its date and real-pilot conditions are satisfied.
