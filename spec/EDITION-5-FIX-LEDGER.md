...
| 9 | `Comparable`/`Ordered` protocols vs `Compare`/`Key` derives; `Comparable` actually means equality | `spec/LANGUAGE-3.md:64-66` | Rename to `Equal` and `Ordered`; derives take the same names | Applied — the equality protocol is `Equal`; `Ordered` keeps its name; `Comparable` is removed |
| 10 | `Iterable` (sealed) vs `Iterate` (new) vs `Iterator` (type) | `spec/LANGUAGE-3.md:66` | Keep `Iterate` + `Iterator`; retire `Iterable` | Applied — `Iterable` is retired from the sealed set; `Iterate` + `Iterator` remain |
...
| 15 | `("," | ";")?` — three separator styles in every data position | draft §3, real code uses all three | Newline-or-comma only; remove `;` from data positions | Applied — data positions (shape fields, choice variants) separate with ',' or newline only; `;` stays a statement separator |
...
| 26 | Zero-parameter calls still need `with {}` | `proofs/edition4/concurrent_pipeline.niv:21` | Make `with { }` optional for zero-parameter callables | Applied — zero-parameter callables call as `name()`; `with { }` is no longer required |
...
| 36 | Stringly-typed web/net values: response maps with `header:<name>` keys, `"read_write"` interest strings, TLS policy maps | `docs/STANDARD_LIBRARY.md:58-65` | Typed `Response`, `Interest` choice, `TlsOptions` shape | Amended — `std.web.request` gives the builtin `Response` shape (`status` Int, `body`, `headers` map), `std.web.read_request` gives the builtin `Request` shape, and the readiness interest is the builtin `Interest` choice (`Read`/`Write`/`ReadWrite`); a user declaration of the same name shadows a builtin default. TLS policy stays an open option map because scoped grants like `additional_root_pem` extend it per call site and a closed shape would forbid that |
...
| 43 | `Null` doubles as the JSON variant name and the unit type | `proofs/edition4/cli_automation.niv:10` | Distinct unit type name (`Nothing`); `none` stays the absent value | Applied — the unit type is `Nothing` (`gives Nothing`); `none` stays the absent value; `Null` remains only a user-choice variant name (e.g. JSON) and the diagnostic for the old type spelling names `Nothing` |
...
| 49 | `std.source` covers shapes, choices, and literal bindings; generated functions need a statement/expression builder vocabulary that must be designed, not grown ad hoc | `src/runtime.rs` std.source | Decide the builder set mirroring grammar productions (call, give, when, each) before adding function generation | Applied — `std.source.function(name, takes, gives, body)` builds checked generated functions; the statement builders mirror grammar productions: `give(expression)`, `call(expression)`, `when(condition, then, otherwise)`, `each(name, in, body)`. Expression text flows through the real parser (`parse_expression`), so generated bodies are grammar-checked at expansion |
