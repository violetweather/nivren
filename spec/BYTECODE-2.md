# Nivren Bytecode and Bundle Specification, Version 2

## 1. Conformance

A version-2 decoder MUST reject malformed, truncated, over-limit, unverifiable, or trailing data before execution. All integers are little-endian. A `count` is `u32` and MUST be no greater than 1,000,000. A string is `count` UTF-8 bytes. Recursive chunk nesting MUST NOT exceed 256.

An application bundle is ASCII `NIVB` followed by exactly one chunk. A chunk contains `u16 bytecode_version (= 2)`, `count instruction_count`, then that many instructions. Every instruction begins with `count line`, `count column`, a `u8` opcode, and its operands.
...
| 19 | DefineRecord | string, strings | define/push record type |
| 20 | DefineEnum | string, strings | define/push enum namespace |
...
`strings` is a count followed by strings. An arm is a variant string, optional-binding tag (`0` or `1` plus string), line, column, and chunk. Nested chunks repeat the version field and MUST be version 2.
...
`Using` MUST perform cleanup on normal return, early return, propagation, and runtime failure. `Propagate` MUST accept only runtime `Result`; malformed bytecode encountering another value fails safely.

Bundles contain no native pointers or host layouts. Optimized execution MUST preserve values, output, errors, call frames, capability checks, resource cleanup, instruction charging, and checked arithmetic. Unsupported versions are rejected rather than guessed.
