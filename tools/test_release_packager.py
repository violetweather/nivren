...
class ReleasePackagerTests(unittest.TestCase):
    def test_archives_are_reproducible_complete_and_executable(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            package_release.package(binary, "0.10.0-beta.6", "linux-x64", first)
            package_release.package(binary, "0.10.0-beta.6", "linux-x64", second)
            ...
            with zipfile.ZipFile(first) as archive:
                ...
                executable = archive.getinfo(
                    "nivren-0.10.0-beta.6-linux-x64/bin/niv"
                )
                self.assertEqual((executable.external_attr >> 16) & 0o777, 0o755)
                installer = archive.getinfo(
                    "nivren-0.10.0-beta.6-linux-x64/install/install.sh"
                )
                self.assertEqual((installer.external_attr >> 16) & 0o777, 0o755)
                for document in package_release.DOCUMENTS:
                    self.assertIn(
                        f"nivren-0.10.0-beta.6-linux-x64/{document}", names
                    )
                self.assertIn(
                    "nivren-0.10.0-beta.6-linux-x64/licenses/INDEX.txt", names
                )
                self.assertIn(
                    "nivren-0.10.0-beta.6-linux-x64/SBOM.spdx.json", names
                )
                self.assertIn(
