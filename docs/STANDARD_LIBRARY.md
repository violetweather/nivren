...
- `std.net.wait_ready(stream, interest, timeout)` waits on the operating system's cross-platform readiness reactor. Interest is `read`, `write`, or `read_write`; `yes` means the caller should attempt the operation and still handle a permitted spurious readiness event, while `no` means the deadline elapsed.
