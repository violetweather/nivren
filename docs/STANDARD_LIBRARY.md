...
- `std.encoding.hex_encode`/`unhex`, `base64`/`unbase64`, and `base64url`/`unbase64url` provide canonical lowercase hexadecimal, padded RFC 4648 base64, and unpadded URL-safe base64. Encoded text is capped at 16 MiB; source bytes are capped at 8 MiB for hex and 12 MiB for base64 so native allocations remain bounded. Decoders reject odd hex, invalid alphabets, noncanonical padding, and oversized output through typed errors.
- `std.map.of`, `set`, `get`, `contains`, `remove`, `length`, `keys`, and `values`
- `std.set.of`, `add`, `contains`, `remove`, `length`, and `values`
...
- `std.net.wait_ready(stream, interest, timeout)` waits on the operating system's cross-platform readiness reactor. Interest is the builtin `Interest` choice — `Interest.Read`, `Interest.Write`, or `Interest.ReadWrite`; `yes` means the caller should attempt the operation and still handle a permitted spurious readiness event, while `no` means the deadline elapsed.
- `std.web.get` needs `Network` and accepts certificate-verified HTTP/HTTPS.
- `std.web.headers()` creates an empty `Map<String,String>`.
- `std.web.request(method, url, headers, body, timeout, maximum)` returns the builtin `Response` shape holding `status` (Int), `body` (String), and `headers` (Map<String, String>). It preserves non-2xx status instead of translating it into a transport error.
...
`std.net.wait_ready(stream, interest, timeout)` waits through the operating-system reactor for one TCP stream. `std.net.wait_ready_any(streams, interest, timeout)` registers up to 1024 streams in one poll and returns the lowest ready array index or `none` at the deadline; an empty array returns `none` immediately. Interests are `Interest.Read`, `Interest.Write`, or `Interest.ReadWrite`, and callers must retry the operation because readiness can be spurious. `std.net.read_ready` waits and then performs one bounded UTF-8 read. `std.net.write_ready` sends an entire value in caller-sized chunks, re-waits under backpressure, and reports timeout with byte progress. TCP and TLS listener accepts use the same readiness mechanism instead of sleep polling.
...
- `std.transactions.create(Map<K,V>)` creates a bounded `Transaction<K,V>` over an immutable snapshot.
