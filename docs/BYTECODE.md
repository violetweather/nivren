...
Edition 4 Intent Proof compiles checked source into portable stack bytecode version 7. The normative format is `spec/BYTECODE-7.md`; earlier versions remain documented as pre-freeze history. Version 7 retains Bytecode 6 derive behavior and adds observable, stack-neutral `Prepare`/`Perform` boundaries plus the zero-extra-dispatch `PerformCall` form.
