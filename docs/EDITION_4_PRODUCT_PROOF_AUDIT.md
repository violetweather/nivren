...
| Daily CLI | In progress | `niv bench` performs isolated warmups/samples and exports `org.nivren.benchmark.v1`; new project templates use Edition 4; bounded `niv-workspace.toml` manifests orchestrate check/build/test/bench/ship in explicit order while retaining per-member incremental fingerprints; targeted CLI tests and warnings-denied clippy pass | Property/fuzz/compatibility UX, deterministic time, dependency-aware workspace scheduling, and broader profiler evidence |
