...
| Daily CLI | In progress | `niv bench` performs isolated warmups/samples and exports `org.nivren.benchmark.v1`; new project templates use Edition 4; bounded `niv-workspace.toml` manifests orchestrate check/build/test/bench/ship; first-class property/compatibility/fuzz-smoke profiles and scoped deterministic-time execution have CLI integration evidence | Dependency-aware workspace scheduling, cache commands, and broader profiler evidence |
