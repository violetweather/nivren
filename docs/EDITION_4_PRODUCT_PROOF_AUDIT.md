...
| Installer recovery | In progress | Unix and Windows installers retain current/previous version receipts, write a machine-readable install receipt, and perform ownership-checked local rollback; the Unix rollback/uninstall test passes and both platform scripts are in the release validation matrix | Independently signed stable/beta/nightly channel manifests, pin/update policy, clean Windows/macOS/Linux machines, and recovery/signing incident drills |
