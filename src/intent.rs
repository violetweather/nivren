...
mod tests {
    ...
    fn explain_is_deterministic_and_orders_effects() {
        let source = "define read_both takes { left is String right is String } gives String or Problem needs FileRead { keep first set perform std.files.read with { path set left } or give\nkeep second set perform std.files.read with { path set right } or give\ngive ok(first + second) }";
